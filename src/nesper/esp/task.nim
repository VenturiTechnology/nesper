...
type
  eTaskState* {.size: sizeof(cint).} = enum
...
type
  eNotifyAction* {.size: sizeof(cint).} = enum
...
type
  TaskStatus_t* {.importc: "TaskStatus_t", header: theader, bycopy.} = object
    ...
    xTaskNumber* {.importc: "xTaskNumber".}: UBaseType_t ## !< A number unique to the task.
...
const
  tskIDLE_PRIORITY* = UBaseType_t(0)
